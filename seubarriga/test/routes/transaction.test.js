...
beforeAll( async () => {
    ...
    await app.db( 'tranfers' ).del();
    await app.db( 'accounts' ).del();
    await app.db( 'users' ).del();
    const users = await app.db( 'users' ).insert( [
        { name: 'User #1', mail: 'user@mail.com', passwd: '$2a$10$Q2WSCcJQquHAPSqKjiSCo.03FWBfGORlQeSPKL4iy61r.VRLFvtiK' },
        { name: 'User #2', mail: 'user2@mail.com', passwd: '$2a$10$Q2WSCcJQquHAPSqKjiSCo.03FWBfGORlQeSPKL4iy61r.VRLFvtiK' },
    ], '*' );
    [ user, user2 ] = users;
    delete user.passwd;
...
describe( 'Ao tentar inserir uma transação inválida', () => {
    ...
    test( 'Não deve inserir sem descrição', () => testTemplate( { ammount: null }, 'Valor é um atributo obrigatório') );
